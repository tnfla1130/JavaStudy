...
public class E06LocalVariable {
	...
	public static void main(String[] args) {
		...
		/*
		main지역에서 num을 선언하면 그보다 좁은 지역이 if문에서는 
		...
		/*
		main보다 좁은 지역인 if문의 블럭을 생성한 후 변수num을 선언한다.
		단, if문과 else문의 블럭은 서로 다른 지역이므로
		동일한 이름의 변수를 선언할 수 있다.
		 */
		if(scope) {
			int num = 1;
			num++;
			System.out.println("첫번쩨 if문 지역:"+num);
	...
	매서드는 크기가 동일한 서로 다른 지역에 정의되었으므로 같은 이름의
	변수를 선언할 수 있다.
